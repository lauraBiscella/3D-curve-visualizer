...
namespace SplineCurves
{
    public class ControlPointsUIPanel : MonoBehaviour
    {
        ...
        void Start()
        {
            inputActive = new bool[requiredPoints];
        ...
        void Update()
        {
            ...
            for(int i=0;i<requiredPoints;i++)
            {
                if(curveSystem.deBoorPoints[i] == null)
                    continue;
                if (!inputActive[i])
                {
                    Vector3 p = curveSystem.deBoorPoints[i].position;
        ...
        {
            if(curveSystem.deBoorPoints[pointIndex] == null)
                return;
            ...
            curveSystem.deBoorPoints[pointIndex].position = new Vector3(x,y,z);
